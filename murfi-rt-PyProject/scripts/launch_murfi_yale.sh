...
then
    ...
    --separator=" " \
    --add-entry="Participant ID (1###) " \
    --add-combo="Step" --combo-values "create_Yale_participant|setup_Yale" \
	...
	--separator=" " \
	--add-combo="Step" --combo-values "setup_Yale|------LOCALIZER-----|2vol|resting_state|extract_rs_networks|process_roi_masks|register|------NEUROFEEDBACK------|2vol|register|feedback|cleanup|backup_reg_mni_masks_to_2vol" \
	--cancel-label "Exit" --ok-label "Run Selected Step"\
...
if [ ${step} == 'create_Yale_participant' ]
then
	echo ${step}
